...
function storeAnswer(question_number, event){
    if(event.target.type === 'radio'){         //Event.target //Uma referência ao objeto que enviou o evento//
        console.log(event.target.value);
        answers['question' +question_number] = parseInt(event.target.value); //Vai alimentar o answers
        console.log(answers);
    }             
...
function getInfoBasedOnScore(){
    if(totalScore() < 7 ){
        ...
        var score_info = "Parabéns! Você está bem informado sobre segurança"
...
submit1.addEventListener('click', function(){
    nextQuestion(2);
    growProgressBar('40%'); // a porcentagem e de acordo do total de perguntas q seram feitas, ate q alcance a marca de 100%
})
